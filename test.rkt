...
; (explicate_tail (parse-exp '(let ([x (let ([x (- 42)]) x) ]) (- x))))

;; (explicate-program (Program '() (parse-exp '(let ([x (let ([y (- 42)]) y) ]) (- x))))
;; output - (Seq (Assign (Var 'y) (Prim '- (list (Int 42)))) (Seq (Assign (Var 'x) (Var 'y)) (Return (Prim '- (list (Var 'x))))))

;; (explicate_tail (parse-exp '(let ([x 10]) x)))

;; (explicate_tail (parse-exp '(let ([x 41]) (+ x 1))))

;; (parse-exp '(+ (let ([x (+ 5 10)]) x) (- 20))

;; (Prim '+ (list (Let 'x (Prim '+ (list (Int 5) (Int 10))) (Var 'x)) (Prim '- (list (Int 20)))))
